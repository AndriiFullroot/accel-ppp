...
struct ppp_t
{
	...
	/** 
	 * In non-dev-ppp environment, channel and unit reads would be perform
	 * through same socket and will yous same handler/function
	 * this flag sets then additionaly would be used unit handlers
	 */
	int is_unit_read_enabled;

	/**
	 * If this flag is set during then, related to /dev/ppp routine would be omited.
	 * The socket `fd` would be used to read chan and unit data.
